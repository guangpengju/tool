    ...
    public String getSqlTemplate() {
        return "DROP TABLE IF EXISTS `${table.name}`; \n" +
            "CREATE TABLE `${table.name}` ( \n" +
                "#[removeLastSep]{" +
                    "#[loop|table.columns]{" +
                        "\t `${item.code}` ${item.type} ${item.size} #[isTrue|item.required]{NOT NULL} " +
                        "#[isFalse|item.required]{DEFAULT #[userDefault|item.defaultValue]{NULL}}" +
                        "#[isTrue|item.required]{#[isEmpty|item.defaultValue]{DEFAULT #[userDefault|item.defaultValue]{NULL}}} " +
